...
export default function EditStudentPage() {
  ...
  const existingEmails = students.filter((s) => s.id !== id).map((s) => s.email.toLowerCase());
  const initialValues: StudentInput = student;

  return (
    <AppLayout>
      <h1>Edit Student</h1>
      <StudentForm isEdit initialValues={initialValues} existingEmails={existingEmails} onSubmit={handleSubmit} />
    </AppLayout>
  );
}