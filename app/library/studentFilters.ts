...
import { SCORE_RANGES } from "@/app/library/constants";
...
export function filterStudents(
  ...
  const range = SCORE_RANGES.find((r) => r.label === filters.scoreRange);
  ...
  return students.filter((student) => {
    ...
    const matchesScore =
      !range || (student.score >= range.min && student.score <= range.max);
